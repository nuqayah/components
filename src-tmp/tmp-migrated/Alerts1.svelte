...
<main>
  <article>
    ...
    <div style="display:flex; gap:0.5rem">
      ...
      <button onclick={() => alerts.error('Invalid email or password.')}>Error</button>
      <button onclick={() => alerts.success('Your settings have been saved.')}>Success</button>
      <button onclick={() => alerts.dismiss_all()}>Dismiss All</button>
    </div>
  </article>
</main>
