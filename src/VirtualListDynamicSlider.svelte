<div class=range-wrapper>
  <output for=page-slider hidden={!range_active} style:--offset={output_offset + 'px'}>{output_text}</output>
  <input
    id=page-slider
    type=range
    min=1
...
function ios_click(e) {
    if (window._useragent.ios)
        virtual_list.go_to_page(Math.floor(get_x_offset_percent(e) * max))
}
