<section class="player-cont">
    ...
    <div class="seek-btns">
        <button
            use:repeat_click={() =>
                (audio.playbackRate = int_clamp(audio.playbackRate - 0.1, 0.4, 4))}
            ><icon id="slower"></icon></button
        >
        <output class="play-speed"
            ><icon id="speed"><span>{audio.playbackRate.toFixed(2)}x</span></icon></output
        >
        <button
            use:repeat_click={() =>
                (audio.playbackRate = int_clamp(audio.playbackRate + 0.1, 0.4, 4))}
            ><icon id="faster"></icon></button
        >
        <span></span>
        <button
            use:repeat_click={() => {
                audio.currentTime += -5
            }}><icon id="skip-back"></icon></button
        >
        <span> </span>
        <button
            use:repeat_click={() => {
                audio.currentTime += 5
            }}><icon id="skip-forward"></icon></button
        >
    </div>
</section>
...
export function set_time(time) {
    if (audio.readyState) audio.currentTime = time
    else {
...
async function set_audio(src) {
    ...
    if (kv && (await kv.get(src))) set_time(await kv.get(src))
}
$: set_audio(src)
...
const ios_seeker_click = e => {
    audio.currentTime = (e.offsetX / e.target.offsetWidth) * duration
}

// Misc
function handle_keydown(e) {
    if (e.key === ' ' && document.activeElement !== play_btn) toggle_playing()
}
